...
def train_model(algo_name="PPO", total_timesteps=1_000_000):
    ...
    eval_env = make_env()
    eval_env = CustomObservationWrapper(eval_env, indices_to_keep=np.array([0,1,2,3,4,6,9,11]))
    eval_env = Monitor(eval_env, log_dir)
    eval_env = DummyVecEnv([lambda: eval_env])

    # Modified callback setup
    checkpoint_callback = CheckpointCallback(
        save_freq=10000,  # Save every 10000 steps (same as eval_freq)
    ...
    model.learn(
        ...
        callback=callbacks  # Updated to use combined callbacks
    )
